// <auto-generated /> <-- to disable warnings in this mostly copy and pasted file
...
/// </summary>
static class ParallelEx
{
    /// <summary>
    /// Gets the default degree of parallelism to use when none is explicitly provided.
    /// </summary>
    static readonly int DefaultDegreeOfParallelism = Environment.ProcessorCount;

    /// <summary>
    /// Executes a for each operation on an <see cref="IAsyncEnumerable{TSource}"/> in which iterations may run in parallel.
    /// </summary>
    ...
    /// The <paramref name="source"/> argument or <paramref name="body"/> argument is <see langword="null"/>.
    /// </exception>
    /// <returns>A task that represents the entire for each operation.</returns>
    /// <remarks>
    /// The operation will execute at most <see cref="Environment.ProcessorCount"/> operations in parallel.
    /// </remarks>
    public static Task ForEachAsync<TSource>(
        IAsyncEnumerable<TSource> source, Func<TSource, CancellationToken, ValueTask> body)
    {
        ...
        Func<TSource, CancellationToken, ValueTask> body)
    {
        ...
        return ForEachAsync(
            source,
            GetEffectiveMaxConcurrencyLevel(parallelOptions),
            GetEffectiveTaskScheduler(parallelOptions),
            parallelOptions.CancellationToken,
            body);
    }

    static TaskScheduler GetEffectiveTaskScheduler(ParallelOptions options)
    {
        return options.TaskScheduler ?? TaskScheduler.Current;
    }

    static int GetEffectiveMaxConcurrencyLevel(ParallelOptions options)
    {
        TaskScheduler scheduler = GetEffectiveTaskScheduler(options);
        int rval = options.MaxDegreeOfParallelism;
        int schedulerMax = scheduler.MaximumConcurrencyLevel;
    ...
    /// <returns>A task that represents the entire for each operation.</returns>
    static Task ForEachAsync<TSource>(
        IAsyncEnumerable<TSource> source,
        int dop,
            ...
            if (_remainingDop > 0)
            {
                ...
                else
                {
                    ...
                    System.Threading.Tasks.Task.Factory.StartNew(
                        _taskBody!, this, default(CancellationToken), TaskCreationOptions.DenyChildAttach, _scheduler);
                }
            }
        }
